...
var month := 1
var year := 1
var rank := "Глава района"
var map_mode := "LOY"

var resources := {"BUD":1200.0, "POL":120.0, "CAD":24.0, "EXP":35.0, "MED":28.0, "debt":180.0}
var districts: Array[Dictionary] = []
var selected_id := ""
var career_score := 52.0
var support_score := 54.0
...
func _recompute_scores() -> void:
	var rdi := weighted_rdi()
	support_score = clamp(0.4 * avg("SERV") + 0.3 * avg("ECO") + 0.2 * resources["MED"] + 0.1 * (100.0 - avg("RISK")), 0.0, 100.0)
	var crisis_score := 100.0 - avg("RISK")
	var loyalty_idx := avg("LOY")
	var federal := clamp(50.0 + (resources["POL"] - 100.0) / 2.0, 0.0, 100.0)
...
func process_month() -> void:
	for d in districts:
		...
		d["LOY"] = clamp(d["LOY"] + 0.15 * ((d["SERV"] - 50.0) / 10.0) + 0.1 * ((d["ECO"] - 50.0) / 10.0) - d["RISK"] / 200.0, 0.0, 100.0)
		d["RISK"] = clamp(d["RISK"] - 0.35 + randf_range(-0.2, 0.5), 0.0, 100.0)

	var avg_eco := avg("ECO")
	var avg_loy := avg("LOY")
	var delta_bud := avg_eco * 12.0 * (0.12 + avg_eco / 500.0) + 30.0 - 12.0 - (55.0 / 9.0)
	resources["BUD"] = clamp(resources["BUD"] + delta_bud, 0.0, 5000.0)
	resources["POL"] = clamp(resources["POL"] + 0.4 * (avg_loy - 50.0) / 10.0, 0.0, 300.0)
	resources["EXP"] = clamp(resources["EXP"] + 0.2, 0.0, 300.0)
	resources["MED"] = clamp(resources["MED"] + 0.15, 0.0, 120.0)

	if randf() < 0.14:
		var idx := randi() % districts.size()
		districts[idx]["RISK"] = clamp(districts[idx]["RISK"] + 6.0, 0.0, 100.0)
		log("Кризисное событие: %s" % districts[idx]["name"])

	month += 1
	if month > 12:
		month = 1
		year += 1
		log("Годовой отчёт сформирован")

	_recompute_scores()
	log("Месяц закрыт: BUD %.1f, POL %.1f, SUP %.1f" % [resources["BUD"], resources["POL"], support_score])

func deploy_crisis_team() -> void:
	if resources["CAD"] < 2:
		log("Недостаточно CAD")
		return
	resources["CAD"] -= 2
	for d in districts:
		d["RISK"] = clamp(d["RISK"] - 1.0, 0.0, 100.0)
	log("Развернута антикризисная команда")
	_recompute_scores()
...
func deploy_eco_team() -> void:
	...
	log("Развернут экономический штаб")
	_recompute_scores()
